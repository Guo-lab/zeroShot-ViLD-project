...
def _generate_detections_v1(boxes,
  ...
  with tf.name_scope('generate_detections'):
    ...
    for i in range(batch_size):
      print("batchsize: [", i, " in ", batch_size, "]")
...
def _generate_detections_per_image(boxes,
  ...
  #//print("\nclass_process: ", num_classes) # 1203
  for i in range(num_classes):
    boxes_i = boxes[:, min(num_classes_for_box - 1, i)]
    scores_i = scores[:, i]
    if i%10==0 :
      print("class_process: [", i, " in ", num_classes, "]")
...
class GenericDetectionGenerator(object):
  ...
  def __call__(self,
    ...
    print("nmsed_boxes(detection_boxed) in vild/ops/postprocess...:", nmsed_boxes)
