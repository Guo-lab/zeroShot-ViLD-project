...
class TpuExecutor(object):
  ...
  def evaluate(self, input_fn, eval_times, checkpoint_path=None):
    ...
    """
    print('tpu_executor.py ', input_fn, eval_times, checkpoint_path)
    
    if not checkpoint_path:
      checkpoint_path = self._estimator.latest_checkpoint()
      print('tpu_executor.py ', checkpoint_path)
    ...
    #* ################################################
      ...
      #!!!!!!
      #current_step = 0
      #checkpoint_path="/Users/gsq/Desktop/tpu/models/official/detection/projects/vild/1mode_dir/origin/model.ckpt-180000"
      print("[tpuexec] in predictor, inputfn: \n", type(input_fn), "\n", input_fn)
      
      saver_hook = tf.train.CheckpointSaverHook(checkpoint_dir="/Users/gsq/Desktop/tpu/models/official/detection/projects/vild/1mode_dir/", checkpoint_basename='model.ckpt', save_secs=5)
      #summary_hook = tf.estimator.SummarySaverHook(summary_op=tf.summary.histogram(name="InPredict", values=),  save_secs=7)
      #stop_hook = tf.estimator.StopAtStepHook()
      step_hook = tf.estimator.StepCounterHook(every_n_steps=1)
      predictor = self._estimator.predict(
          input_fn=input_fn,    
          predict_keys=None,
          hooks=[saver_hook, step_hook],
          checkpoint_path=checkpoint_path,
          yield_single_examples=False)
      
      print("[tpu exe file] predictor: ", predictor)
      print("[tpu exe file] predictor type: ", type(predictor))
      losses = collections.defaultdict(lambda: 0.0)
      print("[tpu exe file] losses: ", losses)
      print("[tpu exe file] losses type: ", type(losses))
      
      counter = 0
      print("GOTO TRY")
      print(eval_times) #! Default 19809!
      
      try:
        ##while eval_times is None or counter < eval_times:
        for counter in range(eval_times):
          print("HERE")
          
          #isit = isinstance(predictor, collections.Iterable)
          #print(isit) #TRUE
          print("[tpu exe file] in eval before iter the predictor: ", predictor, '\nTRAPPED in predictor\n')
          ...
          self._evaluator.update(
              predictions,
              groundtruths=(None if self._params.eval.use_json_file else groundtruths))
        ...
        logging.info(
            'Meet Error {tf.errors.OutOfRangeError, StopIteration}')
